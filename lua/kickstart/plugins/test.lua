...
return {
    ...
    keys = {
        ...
        { "]e",          function() require("neotest").jump.next({ status = 'failed' }) end, desc = "Failed Test Forward",                                        ft = ft },
        { "[e",          function() require("neotest").jump.prev({ status = 'failed' }) end, desc = "Failed Test Backward",                                       ft = ft },
        { "]E",          function() require("neotest").jump.prev({ status = 'failed' }) end, desc = "Failed Test Backward",                                       ft = ft },
        { "[E",          function() require("neotest").jump.next({ status = 'failed' }) end, desc = "Failed Test Forward",                                        ft = ft },
        -- Gopher mappings
